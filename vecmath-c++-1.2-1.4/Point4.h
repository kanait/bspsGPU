 ...
 * @version specification 1.1, implementation $Revision: 1.1.1.1 $, $Date: 2005/04/01 17:12:52 $
...
class Point4 : public Tuple4<T> {
  /*
   * $Log: Point4.h,v $
   * Revision 1.1.1.1  2005/04/01 17:12:52  kanai
   *
   *
   * Revision 1.1.1.1  2002/11/11 17:05:17  kanai
   *
   *
   * cvs に登録しました．
   *
 ...
 public:
  ...
  T distanceSquared(const Point4& p1) const {
    T dx = Tuple4<T>::x - p1.x;
    T dy = Tuple4<T>::y - p1.y;
    T dz = Tuple4<T>::z - p1.z;
    T dw = Tuple4<T>::w - p1.w;
  ...
  T distanceL1(const Point4& p1) const {
    return (   VmUtil<T>::abs(Tuple4<T>::x-p1.x)
             + VmUtil<T>::abs(Tuple4<T>::y-p1.y)
             + VmUtil<T>::abs(Tuple4<T>::z-p1.z)
             + VmUtil<T>::abs(Tuple4<T>::w-p1.w) );
  }
  ...
  T distanceLinf(const Point4& p1) const {
    return ( VmUtil<T>::max(VmUtil<T>::abs(Tuple4<T>::x-p1.x),
                            VmUtil<T>::abs(Tuple4<T>::y-p1.y),
                            VmUtil<T>::abs(Tuple4<T>::z-p1.z),
                            VmUtil<T>::abs(Tuple4<T>::w-p1.w)) );
  }
  ...
  void project(const Point4& p1) {
    // zero div may occur.
    Tuple4<T>::x = p1.x/p1.w;
    Tuple4<T>::y = p1.y/p1.w;
    Tuple4<T>::z = p1.z/p1.w;
    Tuple4<T>::w = 1;
  }
