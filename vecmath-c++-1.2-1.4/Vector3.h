...
class Vector3 : public Tuple3<T> {
   ...
   *
   *
   * Revision 1.3  2004/03/26 20:07:42  kanai
   * no message
   *
   * Revision 1.2  2004/02/28 14:07:05  kanai
   * no message
   *
 ...
 public:
  ...
  T dot(const Vector3& v1) const {
    return Tuple3<T>::x*v1.x + Tuple3<T>::y*v1.y + Tuple3<T>::z*v1.z;
  }
  ...
  T lengthSquared() const {
    return ( Tuple3<T>::x*Tuple3<T>::x +
             Tuple3<T>::y*Tuple3<T>::y +
             Tuple3<T>::z*Tuple3<T>::z );
  }
  ...
  T cosign(const Vector3& v1) const {
