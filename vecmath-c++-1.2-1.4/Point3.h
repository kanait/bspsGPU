...
class Point3 : public Tuple3<T> {
 ...
 public:
  ...
  T distanceSquared(const Point3& p1) const {
    T dx = Tuple3<T>::x - p1.x;
    T dy = Tuple3<T>::y - p1.y;
    T dz = Tuple3<T>::z - p1.z;
  ...
  T distanceL1(const Point3& p1) const {
    return ( VmUtil<T>::abs(Tuple3<T>::x-p1.x)
             + VmUtil<T>::abs(Tuple3<T>::y-p1.y)
             + VmUtil<T>::abs(Tuple3<T>::z-p1.z) );
  }
  ...
  T distanceLinf(const Point3& p1) const {
    return ( VmUtil<T>::max(VmUtil<T>::abs(Tuple3<T>::x-p1.x),
                            VmUtil<T>::abs(Tuple3<T>::y-p1.y),
                            VmUtil<T>::abs(Tuple3<T>::z-p1.z)) );
  }
  ...
  void project(const Point4<T>& p1) {
    // zero div may occur.
    Tuple3<T>::x = p1.x/p1.w;
    Tuple3<T>::y = p1.y/p1.w;
    Tuple3<T>::z = p1.z/p1.w;
  }
...
typedef Point3<unsigned short> Point3us;
