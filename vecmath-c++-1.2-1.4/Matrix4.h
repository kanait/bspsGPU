...
template<class T>
void Matrix4<T>::transform(const Tuple4<T>& vec_org, Tuple4<T>* vecOut) const {
	// alias-safe
    assert(vecOut != 0);
	vecOut->set(
	    m00*vec_org.x + m01*vec_org.y + m02*vec_org.z + m03*vec_org.w,
	    m10*vec_org.x + m11*vec_org.y + m12*vec_org.z + m13*vec_org.w,
	    m20*vec_org.x + m21*vec_org.y + m22*vec_org.z + m23*vec_org.w,
	    m30*vec_org.x + m31*vec_org.y + m32*vec_org.z + m33*vec_org.w
	    );
}
