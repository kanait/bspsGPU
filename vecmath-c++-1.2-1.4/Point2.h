...
class Point2 : public Tuple2<T> {
 ...
 public:
  ...
  T distanceSquared(const Point2& p1) const {
    T dx = Tuple2<T>::x - p1.x;
    T dy = Tuple2<T>::y - p1.y;
    return dx*dx + dy*dy;
  }
  ...
  T distanceL1(const Point2& p1) const {
    return VmUtil<T>::abs(Tuple2<T>::x-p1.x) + VmUtil<T>::abs(Tuple2<T>::y-p1.y);
  }
  ...
  T distanceLinf(const Point2& p1) const {
    return VmUtil<T>::max(VmUtil<T>::abs(Tuple2<T>::x-p1.x), VmUtil<T>::abs(Tuple2<T>::y-p1.y));
  }
