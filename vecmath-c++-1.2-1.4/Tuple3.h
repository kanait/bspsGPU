 ...
 *
 *
 * Revision 1.2  2004/02/25 09:22:49  kanai
 * no message
 *
...
public:
    ...
    bool zero() const {
      if ( (VmUtil<T>::abs(x) < 1.0e-06) &&
	   (VmUtil<T>::abs(y) < 1.0e-06) &&
	   (VmUtil<T>::abs(z) < 1.0e-06) ) return true;
      return false;
    }
  ...
  /**
    ...
    T getMaxLen() {
      T tmp = (VmUtil<T>::abs(x) > VmUtil<T>::abs(y) ) ? VmUtil<T>::abs(x) : VmUtil<T>::abs(y); 
      return ( tmp > VmUtil<T>::abs(z) ) ? tmp : VmUtil<T>::abs(z);
    }
